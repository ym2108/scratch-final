...
export default function App() {
  ...
  const divideSize = (percentage) => {
    setCatSize((prevSize) => prevSize * (percentage / 100));
  };
  ...
  const rotateSprite = (angle) => {
    // Update the rotation angle of the cat
  ...
  const bounceCat = () => {
    ...
    const catWidth = 100; // Assuming the width of the cat sprite is 100px
    const catHeight = 100; // Assuming the height of the cat sprite is 100px
    ...
    <div className="bg-blue-100 pt-6 font-sans">
      <div className="h-screen overflow-hidden flex flex-row  ">
        <div className="flex-1 h-screen overflow-hidden flex flex-row bg-white border-t border-r border-gray-200 rounded-tr-xl mr-2">
          <Sidebar moveSprite={moveSprite}  increaseSize={increaseSize} divideSize={divideSize} 
          rotateSprite={rotateSprite} bounceCat={bounceCat} onCheckboxChange={handleCheckboxChange} onSayClick={handleSayClick} OnPref={handlePref}  onShowCat={handleShowCat} onHideCat={handleHideCat}/>
          <MidArea />
        </div>
        <div className="w-1/3 h-screen overflow-hidden flex flex-row bg-white border-t border-l border-gray-200 rounded-tl-xl ml-2">
          <PreviewArea catSize={catSize} setMessageDisplayed={setMessageDisplayed} pref={pref} flag={flag} flag1={flag1} message={message} duration={duration} xAxisChecked={xAxisChecked} yAxisChecked={yAxisChecked} directionChecked={directionChecked}  catPosition={catPosition} catRotation={catRotation} moveSprite={moveSprite} previewAreaSize={previewAreaSize} />
        </div>
      </div>
    </div>
  );
